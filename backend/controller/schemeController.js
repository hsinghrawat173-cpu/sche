...
//     // 🔒 THE SMART BOUNCER LOGIC
//     // Clean up the user data to prevent Javascript glitching
//     const safeOccupation = (userProfile.occupation || "General").trim().toLowerCase();
//     const safeIncome = Number(userProfile.income) || 0;

//     let preFilteredSchemes = allSchemes.filter(scheme => {
//       const schemeCategory = (scheme.targetCategory || "General").trim().toLowerCase();
//       const schemeLimit = Number(scheme.incomeLimit) || 0;

//       const matchOcc = schemeCategory === safeOccupation || schemeCategory === "general";
//       const matchInc = schemeLimit === 0 || safeIncome <= schemeLimit;

//       return matchOcc && matchInc;
//     });

//    console.log(`🚨 TRIPWIRE 2: Found ${allSchemes.length} schemes. Slimming the payload...`);

//     // NO PRE-FILTERING. Let the AI do the thinking.
//     // Just strip the heavy MongoDB data to prevent payload choke.
...
//         For each one, add a field "aiReason" explaining in 1 short sentence why it fits this user.
//         Output ONLY valid JSON without any markdown formatting blocks.
//       `;

//       const result = await model.generateContent(prompt);
//       const text = result.response.text().replace(/```json|```/g, "");
//       const aiMatchedSchemes = JSON.parse(text);

//       console.log("🚨 TRIPWIRE 4: Google Responded Successfully!");
//       return res.status(200).json({ success: true, matchedSchemes: aiMatchedSchemes });

//     } catch (geminiError) {
//       console.error("❌ Google API Failed. Triggering Fallback...");
//       const fallbackSchemes = slimPayload.map(scheme => ({
//         ...scheme._doc,
//         aiReason: `Matched based on your profile as a ${safeOccupation}. (Standard Match)`
//       }));
//       return res.status(200).json({ success: true, matchedSchemes: fallbackSchemes });
//     }
...
//   console.log("DATABASE TEST:", allSchemes[0].title, allSchemes[0].targetCategory, allSchemes[0].incomeLimit);
// };
